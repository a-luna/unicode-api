from tests.test_character_endpoints.test_list_all_unicode_characters.data import (
...
def test_block_miao_start_after_def_limit_def(client):
    response = client.get("/v1/characters?block=MIAO")
    assert response.status_code == 200
    assert response.json() == BLOCK_MIAO_START_AFTER_DEF_LIMIT_DEF


def test_counting_rod_numerals_end_before_1d380(client):
...
def test_block_thai_start_after_invalid(client):
...
def test_invalid_block_name_no_fuzzy_matches(client):
